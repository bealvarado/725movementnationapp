...
async function getBooking(req, res) {
  try {
    ...
    if (!uid) {
      console.error("Missing UID in query parameters.");
    ...
    if (bookingSnapshot.empty) {
      console.log("No bookings found for this UID.");
      return res.status(404).json({ message: "No bookings found for this user." });
    }

    const bookings = bookingSnapshot.docs.map((doc) => ({
      id: doc.id, // Include Firestore document ID
      ...doc.data(), // Include all booking data
    }));

    console.log(`Found ${bookings.length} bookings for UID: ${uid}`);
